...
@torch.no_grad()
def factor_function_test_per_row_wider_gauss(param:nn.Parameter):
...
def inject_scale_layers_no_factor(
    ...
    factor_function=None, # can be none, here for compatability with normal inject_scale_layers
...
def rebalance_layers_wdd(
    ...
    for name, child in model.named_children():
        if isinstance(child,nn.Sequential) and (len(child) == 2) and isinstance(child[1],ScaleLayer):
            ...
            wdd_i = (param_dict[linear.weight].get("wdd_index",None))
            if wdd_i is not None:
                max_lrs[wdd_i]*=((1 / factors.mean()) ** 2).to(dtype_target).item()
                base_wds[wdd_i] *= (factors.mean() ** 2).to(dtype_target).item()
                print(f"new: {wdd_i=}, {max_lrs[wdd_i]=} {base_wds[wdd_i]=}")
            # BatchNorm scale: stays the same
        else:
            rebalance_layers_wdd(
                child,
...
def main():
    ...
    def longrun_scale_layer(
        ...
        try:
            ...
            inject_scale_layers_no_factor(
                model,
                param_groups,
                factor_function=factor_function_test_per_row_wider_gauss,
                learning_rate=learning_rate,
                weight_decay=weight_decay,
                scale_grad=False,
            )
            ...
            for param_group in param_groups:
                ...
                # all should be single param param groups.
                if param.ndim > 1:
                    if "fc" in names[param]:
                        print("is a matrix not normalized: ",names[param])
                    else:
                        print("is a matrix wdd weight:",names[param])
                        param_groups_wdd.append(param_group)
                else:
                    print("is not a matrix weight: ",names[param])
            ...
            max_lrs = [learning_rate] * len(base_wds)# alpha / (1-beta)
            print("max_lrs only initialized for no_factor_init")
            ...
            with wandb.init(
                ...
                for epoch in range(0, num_epochs):
                    ...
                    if epoch == REG_wait_epochs - 1:  # TODO: check one off error
                        ...
                        for index,group in enumerate(param_groups_wdd):
                            param = group["params"][0]
                            param_dict[param]["wdd_index"]=index
    ...
    hist, model, optimizer = longrun_scale_layer(
        ...
        "factor_one_init_rebalance_per_row_wdd_gauss",
    )
